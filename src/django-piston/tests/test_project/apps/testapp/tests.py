...
class BasicAuthTest(MainTests):
    ...
    def test_invalid_auth_header(self):
        response = self.client.get('/api/basic_auth/entries/')
...
class DjangoAuthTest(TestCase):
    ...
    def test_annonymous_user(self):
        response = self.client.get('/api/django_auth/entries/',follow=True)
        self.assertEquals(response.redirect_chain,
                          [('http://testserver/accounts/login/?next=/api/django_auth/entries/',
                            302)]
                          )
    def test_authenticated_user(self):
        self.client.login(username='joe',
                          password='secret')
        response = self.client.get('/api/django_auth/entries/')
...
class MultiXMLTests(MainTests):
    ...
    def test_multixml(self):
        ...
        result = self.client.get('/api/basic_auth/entries.xml',
    ...
    def test_singlexml(self):
        ...
        result = self.client.get('/api/basic_auth/entry-%d.xml' % (obj.pk,),
                HTTP_AUTHORIZATION=self.auth_string).content
        self.assertEquals(expected, result)
...
class AbstractBaseClassTests(MainTests):
    ...
    def test_field_presence(self):
        result = self.client.get('/api/basic_auth/abstract.json',
                HTTP_AUTHORIZATION=self.auth_string).content
    ...
    "some_field": "something here"
        ...
        for id_ in ids:
            result = self.client.get('/api/basic_auth/abstract/%d.json' % id_,
                    HTTP_AUTHORIZATION=self.auth_string).content
...
class IncomingExpressiveTests(MainTests):
    ...
    }, 
    {
        ...
        result = self.client.get('/api/basic_auth/expressive.json',
            HTTP_AUTHORIZATION=self.auth_string).content

        self.assertEquals(result, expected)
        
        resp = self.client.post('/api/basic_auth/expressive.json', outgoing, content_type='application/json',
    ...
    def test_incoming_invalid_json(self):
        resp = self.client.post('/api/basic_auth/expressive.json',
            'foo',
            HTTP_AUTHORIZATION=self.auth_string,
...
"""
        ...
        self.assertEquals(self.client.get('/api/basic_auth/expressive.yaml',
        ...
        resp = self.client.post('/api/basic_auth/expressive.json', outgoing, content_type='application/x-yaml',
            HTTP_AUTHORIZATION=self.auth_string)
...
"""
    ...
    def test_incoming_invalid_yaml(self):
        resp = self.client.post('/api/basic_auth/expressive.yaml',
            '  8**sad asj lja foo',
            HTTP_AUTHORIZATION=self.auth_string,
...
class Issue36RegressionTests(MainTests):
    ...
    def test_simple(self):
        ...
        if True:
            fp = open(__file__, 'r')
            try:
                response = self.client.post('/api/basic_auth/entries.xml',
        ...
        fp = open(__file__, 'r')
        try:
            response = self.client.put('/api/basic_auth/entry-%d.xml' % self.data.pk,
                    {'file': fp}, HTTP_AUTHORIZATION=self.auth_string)
...
class ValidationTest(MainTests):
    def test_basic_validation_fails(self):
        ...
        self.assertEquals(resp.content,
                          'Bad Request {"msg": ["This field is required."]}')
        
    def test_basic_validation_xml_output_fails(self):
        resp = self.client.get('/api/echo', {'format':'xml'})
        self.assertEquals(resp.status_code, 400)
        self.assertEquals(resp.content, 'Bad Request <ul class="errorlist">'
            '<li>msg<ul class="errorlist"><li>This field is required.</li>'
            '</ul></li></ul>')
        
    def test_basic_validation_json_output_fails(self):
        resp = self.client.get('/api/echo', {'format':'json'})
        self.assertEquals(resp.status_code, 400)
        self.assertEquals(resp.content,
                          'Bad Request {"msg": ["This field is required."]}')
        
    def test_basic_validation_yaml_output_fails(self):
        if not yaml:
            return
        resp = self.client.get('/api/echo', {'format':'yaml'})
        self.assertEquals(resp.status_code, 400)
        self.assertEquals(resp.content,
                          "Bad Request msg: [!!python/unicode 'This field is required.']\n")
