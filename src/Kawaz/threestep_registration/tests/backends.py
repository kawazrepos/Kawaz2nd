...
def _mock_site():
    if Site._meta.installed:
        ...
        site = RequestSite(_mock_request())
    return site

class ThreeStepRegistrationBackendTests(TestCase):
    """
    ...
    def test_registration(self):
        ...
        # and Notification email was sent.
        self.assertEqual(RegistrationProfile.objects.count(), 1)
        self.assertEqual(len(mail.outbox), 2)
        
        registration_profile = RegistrationProfile.objects.get(user=new_user)
        self.assertEqual(registration_profile.remarks, 'remarks')
    ...
    def test_valid_approvation(self):
        ...
        success, failed = self.backend.approve(_mock_request(),
                                          [valid_user.username])
        self.assertEqual(len(failed), 0)
        self.assertEqual(len(success), 1)
        self.failIf(success[0].is_active)
        
        # Fetch the profile again to verify its activation key has
        # been reset.
        valid_profile = RegistrationProfile.objects.get(user=valid_user)
        self.assertEqual(valid_profile.status, 'approved')
        
        # Two for registration, one for approvation
    ...
    def test_valid_rejection(self):
        ...
        success, failed = self.backend.reject(_mock_request(),
                                          [valid_user.username])
        self.assertEqual(len(failed), 0)
        self.assertEqual(len(success), 1)
        self.failIf(success[0].is_active)
        
        self.failIf(User.objects.filter(username=valid_user.username).exists())
        self.failIf(RegistrationProfile.objects.filter(user=valid_user).exists())
        
        # Two for registration, one for rejection
        self.assertEqual(len(mail.outbox), 3)
    ...
    def test_valid_activation(self):
        ...
        # Two for registration, One for activation
        self.assertEqual(len(mail.outbox), 3)
    ...
    def test_form_class(self):
        ...
        """
        self.failUnless(self.backend.get_form_class(_mock_request()) is forms.RegistrationFormTermsOfService)
    ...
    def test_email_send_action(self):
        ...
        self.assertEqual(len(mail.outbox), 3) # Two on registering, one more on the resend.
        
        RegistrationProfile.objects.filter(user=alice).update(activation_key=RegistrationProfile.ACTIVATED)
        admin_class.resend_activation_email(_mock_request(),
                                            RegistrationProfile.objects.all())
        self.assertEqual(len(mail.outbox), 3) # No additional email because the account has activated.
