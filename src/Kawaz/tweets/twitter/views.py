...
def unauth(request):
    #
    # Notice:
    #    Twitter上で許可を取り消すのはConsumer側からは不可能？
    #    仕方がないのでとりあえずアクセストークン削除するだけにとどめておく
    #
    # 不要
...
def return_(request):
    ...
    #アクセストークンをユーザープロフィールに格納して保存しちゃう！
    user = request.user
    if user.is_authenticated():
        profile = user.get_profile()
        profile.twitter_token = access_token.to_string()
        profile.save()
    return response
...
def friend_list(request):
    ...
    if auth:
        ...
        for page in range(pages):
            ...
            users.append(json)
    return render_to_response('twitter_app/list.html', {'users': users, 'user': request.user}) 